...
public class SimpleHttp {
    ...
    private void response(SelectionKey selectionKey) throws IOException {
        ...
        if (headers.getPath().startsWith(META_RESOURCE_DIR_PREFIX)) {
            ...
            channel.close();
            log(ip, headers, StatusCode.FORBIDDEN.getCode());
            return;
        }

        try {
            handleOK(channel, headers.getPath());
            log(ip, headers, StatusCode.OK.getCode());
        } catch (FileNotFoundException e) {
            handleNotFound(channel);
            log(ip, headers, StatusCode.NOT_FOUND.getCode());
        } catch (Exception e) {
            handleInternalServerError(channel);
            log(ip, headers, StatusCode.INTERNAL_SERVER_ERROR.getCode());
        } finally {
            channel.close();
        }
    }
    ...
    private void handleOK(SocketChannel channel, String path) throws IOException {
        ResponseHeaders headers = new ResponseHeaders(StatusCode.OK.getCode());

        ByteBuffer bodyBuffer = readFile(path);
        headers.setContentLength(bodyBuffer.capacity());
        headers.setContentType(parseContentType(getExtension(path)));
    ...
    private void handleError(SocketChannel channel, int statusCode) throws IOException {
        ...
        headers.setContentLength(bodyBuffer.capacity());
        headers.setContentType(parseContentType("html"));
        ByteBuffer headerBuffer = ByteBuffer.wrap(headers.toString().getBytes());
    ...
    private String parseContentType(String key) throws IOException {
        return (String) JSONParseUtil.getJsonObject().get(key);
    }

    public static void main(String[] args) throws IOException {
        new SimpleHttp().start();
    }
}
